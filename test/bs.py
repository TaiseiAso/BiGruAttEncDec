    ...
    for input, output in dialog_corpus:
        with open(test_log_name, 'a', encoding='utf-8') as f:
            ...
            for i in range(5):
                ...
                for j in range(6):
                    b = 2 ** j
                    beam_ress = beam_search(decoder, hs, h, glove_vectors, target_dict, device, rep_sup=0.4, B=b, length_norm=ln)
                    f.write("beam search 0.4 " + str(b) + " {:2.1f}".format(ln) + ":" + ' '.join(reranking(beam_ress)) + "\n")
